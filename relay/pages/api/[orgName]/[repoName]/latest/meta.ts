...
const getLatestReleaseMeta = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (publicRuntimeConfig.WEB3_PROVIDER) {
    ...
    if (latestRelease) {
      ...
      res.redirect(`https://gateway.valist.io/ipfs/${latestRelease.metaCID}`);
    } else {
...
export default withSentry(getLatestReleaseMeta);