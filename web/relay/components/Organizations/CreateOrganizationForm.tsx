...
interface CreateOrganizationFormProps {
  createOrg: (shortName: string, meta: OrgMeta) => Promise<void>
}

export default function CreateOrganizationForm(props: CreateOrganizationFormProps): JSX.Element {
  const [shortName, setShortName] = useState('');
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');

  const submit = async () => {
    if (shortName && name && description) {
      await props.createOrg(shortName, { name, description });
    } else {
  ...
  const updateShortname = (shortname: string) => {
    setShortName(shortname.toLowerCase().replace(shortnameFilterRegex, ''));
  };
              ...
              <form className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                  <div className="col-span-12">
                      ...
                      <div className="mt-1 flex rounded-md shadow-sm">
                          ...
                          sm:leading-5" type="text" placeholder="my-organization" value={shortName}/>
                      </div>
